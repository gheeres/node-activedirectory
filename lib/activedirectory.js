...
ActiveDirectory.prototype.findUser = function() {
  var args = arguments, argl = args.length, self = this, opts, username, 
      includeMembership, callback = _.last(arguments);

  /**
   * Verifies if the method are being used correctly.
   *  
   * findUser('username', callback())
   * findUser({}, 'username', callback())
   * findUser({}, 'username', true, callback())
   */
  ...
  if (typeof args[0] === "string" && argl === 2) {
    ...
    if (argl === 3) { 
      opts = args[0];
      username = args[1];
    } else if (typeof args[2] === "boolean" && argl === 4) { 
      opts = args[0];
      username = args[1];
      includeMembership = args[2];
    }
  } else {
    callback("invalid method signature!");
  }
