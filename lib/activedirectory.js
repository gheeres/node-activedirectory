...
function ActiveDirectory(options) { // jshint -W071
  ...
  log.info('Referrals are %s', defaultReferrals.enabled ?
    'enabled. Exclusions: '+JSON.stringify(defaultReferrals.exclude) : 'disabled'
  );
...
ActiveDirectory.prototype.getGroupMembershipForGroup = function getGroupMembershipForGroup(opts, groupName, callback) {
  ...
  this.getGroupDistinguishedName(_opts, _groupName, (err, dn) => {
    ...
    if (!dn) {
      log.warn('Could not find a distinguishedName for the specified group name: "%s"', _groupName);
      return _cb();
    }
...
ActiveDirectory.prototype.findDeletedObjects = function find(opts, callback) {
  ...
  const defaultDeletedAttributes = [ // jshint -W100
  ...
  const searchDeletedObjects = (baseDN, options) => {
    const innerOpts = Object.assign({}, options, {includeDeleted: true});
    search(baseDN, innerOpts, (err, results) => {
      ...
      if (results.length === 0) {
        log.warn('No deleted objects found for query "%s"', utils.truncateLogOutput(_opts.filter));
        self.emit('done', null);
        return _cb();
      }
  ...
  if (!localOpts.baseDN) {
    log.debug('No baseDN specified for Deleted Object. Querying RootDSE at %s.', this.opts.url);
    this.getRootDSE(this.opts.url, ['defaultNamingContext'], (err, result) => {
      if (err) {
        return _cb(err);
      }

      log.info('Retrieved defaultNamingContext (%s) from RootDSE at %s.', result.defaultNamingContext, this.opts.url);
 ...
 * @param {function} callback The callback to execute when completed.
 */
ActiveDirectory.prototype.findUser = function findUser(opts, username, includeMembership, callback) { // jshint -W071
  'use strict';
