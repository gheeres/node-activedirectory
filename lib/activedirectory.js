...
var ActiveDirectory = function(url, baseDN, username, password, defaults) {
  if (this instanceof ActiveDirectory) {
    ...
    if (typeof(url) === 'string') {
      ...
      defaultEntryParser = typeof defaults.entryParser === "function" ? defaults.entryParser : defaultEntryParser;
    }
    else {
      this.opts = url;
...
ActiveDirectory.prototype.authenticate = function authenticate(username, password, callback) {
  ...
  client.on('error', function(err) {
    ...
    if ((err || {}).errno !== 'ECONNRESET') {
      log.error('Authentication failed for "%s" as "%s" (password: "%s") due to an unknown error: %s.', 
...
ActiveDirectory.prototype.getRootDSE = function getRootDSE(url, attributes, callback) {
  ...
  function onError(err) {
    // Ignore ECONNRESET errors
    if ((err || {}).errno !== 'ECONNRESET') {
      var message = util.format('An unhandled error occured when searching for the root DSE at "%s". Error: %s', url, err);
      log.warn(message);
      if (hasEvents('error')) self.emit('error', message)
      return(callback(err));
    }
  }
  ...
  client.bind('', '', function(err) {
    if (err) {
      log.warn('Anonymous bind to "%s" failed. Error: %s', url, err);
      return(callback(err, false));
    }

    client.search('', { scope: 'base', attributes: attributes || [ '*' ], filter: '(objectClass=*)' }, function(err, result) {
      if (err) {
        log.warn('Root DSE search failed for "%s". Error: %s', url, err);
        return(callback(err));
      }
