...
ActiveDirectory.prototype.isUserMemberOf = function isUserMemberOf(opts, username, groupName, callback) {
  ...
  self.getGroupMembershipForUser(opts, username, function(err, groups) {
    ...
    // retrieved groups.
    var lowerCaseGroupName = (groupName || '').toLowerCase().replace(/\s/g, '');
    var result = _.any(groups, function(item) {
      return(((item.dn || '').toLowerCase().replace(/\s/g, '') === lowerCaseGroupName) ||
             ((item.cn || '').toLowerCase().replace(/\s/g, '') === lowerCaseGroupName));
     });
...
ActiveDirectory.prototype.find = function find(opts, callback) {
  ...
  const search = require('./components/search')(self, log);
  search(localOpts, function onFind(err, results) {
    if (err) {
      if (callback) callback(err);
      return;
    }
