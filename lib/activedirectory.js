...
//    { level: 'info',
//      stream: '/var/log/activedirectory.log' }
  ]
});
...
var defaultEntryParser = function(entry, callback) {
  if (callback) {
...
var defaultFindOpts = {
  includeUsersMembership : false,
  includeGroupsMembership : false,
  ...
  if (this instanceof ActiveDirectory) {
    ...
    if (typeof(url) === 'string') {
      ...
      if (typeof((defaults || {}).entryParser) === 'function') {
        defaultEntryParser = defaults.entryParser;
      }
    }
    else {
      this.opts = url;
      ...
      if (typeof(this.opts.entryParser) === 'function') {
        defaultEntryParser = this.opts.entryParser;
      }
...
function search(baseDN, opts, callback) {
  ...
  function onSearchEntry(entry) {
    ...
    parseRangeAttributes.call(self, result, opts, function(err, item) {
      ...
      if (err) item = result;
      defaultEntryParser(entry, function(item){
        results.push(item);
  ...
  function onReferralChase(ref) {
    ...
    while (referralUrl = (ref.uris || [])[index++]) {
      if (isAllowedReferral(referralUrl)) {
        ...
        referralClient.search(referralClient.url.DN, parseLdapJSSearchOpts(opts), controls, function(err, res) {
          /**
  ...
  /**
   * Parsesthe ldapjs options to pass into the search.
   * @param {Object} opts The existing options to parse and make ready for ldapjs library
   * @returns {Object} The updated options.
   */
  function parseLdapJSSearchOpts(opts) {
    return(_.omit(opts || {}, 'baseDN', 'controls'));
  }
  ...
  log.debug('Querying active directory (%s) with filter "%s" for %j',
            baseDN, truncateLogOutput(opts.filter), _.any(opts.attributes) ? opts.attributes : '[*]');
  client.search(baseDN, parseLdapJSSearchOpts(opts), controls, function onSearch(err, res) {
    if (err) {
...
function getGroupMembershipForDN(opts, dn, stack, callback) {
  ...
  log.trace('getGroupMembershipForDN(%j,%s,%j)', opts, dn, stack);
  ...
  var opts = _.defaults(opts || {}, {
    ...
    scope: 'sub',
    attributes: _.union([ 'objectCategory', 'groupType', 'distinguishedName', 'cn' ], defaultAttributes.group)
  });
  search.call(self, opts, function(err, results) {
    ...
    async.forEach(results, function(group, asyncCallback) {
      ...
      if (isGroupResult(group)) {
        ...
        getGroupMembershipForDN.call(self, group.dn, groups, function(err, nestedGroups) {					
          ...
          log.debug('Group "%s" which is a member of group "%s" has %d nested group(s). Groups: %j',
                    group.dn, dn, nestedGroups.length, nestedGroups);
      ...
      log.info('Group "%s" has %d group(s). Groups: %j', dn, groups.length, groups);
      callback(err, groups);
    });
  });
}
  ...
  self.findGroup(opts, groupName, function(err, group) {
    ...
    async.each(chunks, function getUsersForGroup_ChunkItem(members, asyncChunkCallback) {
      ...
      var opts = {
        filter: filter,
        scope: 'sub',
        attributes: _.union(defaultAttributes.user || [], [ 'groupType' ])
      };
...
ActiveDirectory.prototype.getGroupMembershipForUser = function getGroupMembershipForUser(opts, username, callback) {
  ...
  getUserDistinguishedName.call(self, opts, username, function(err, dn) {
    ...
    getGroupMembershipForDN.call(self, opts, dn, function(err, groups) {
      ...
      if (callback) callback(err, groups);
    });
  });
};
...
ActiveDirectory.prototype.userExists = function userExists(opts, username, callback) {
  ...
  self.findUser(opts, username, function(err, user) {
    ...
    log.info('"%s" %s exist.', username, (result != null) ? 'DOES' : 'DOES NOT');
 ...
 */
ActiveDirectory.prototype.find = function find(opts, findOpts, callback) {
  var self = this;

  if (typeof(findOpts) === 'function') {
    callback = findOpts;
  }
  if (typeof(opts) === 'function') {
    callback = opts;
  ...
  var findOpts = _.defaults(findOpts || {}, defaultFindOpts);

  log.trace('find(%j,%j)', opts, findOpts);

  var opts = _.defaults(opts || {}, {
    scope: 'sub',
    attributes: _.union(defaultAttributes.group || [], defaultAttributes.user || [], [ 'objectCategory' ])
  });
  search.call(self, opts, function onFind(err, results) {
    ...
    if ((! results) || (results.length === 0)) {
      log.warn('No results found for query "%s"', truncateLogOutput(opts.filter));
      if (callback) callback();
      self.emit('done');
      return;
    }
    ...
    async.forEach(results, function(item, asyncCallback) {
      if (isGroupResult(item)) {
        ...
        if (findOpts.includeGroupsMembership) {
          getGroupMembershipForDN.call(self, group.dn, function(err, groups) {
        ...
        // Also retrieving user group memberships?
        if (findOpts.includeUsersMembership) {
...
ActiveDirectory.prototype.findDeletedObjects = function find(opts, callback) {
  ...
  function searchDeletedObjects(baseDN, opts) {
    if (!_.any(opts.controls, function(control) { return(control.type === '1.2.840.113556.1.4.417'); })) {
      log.debug('Adding ShowDeletedOidControl(1.2.840.113556.1.4.417) to search (%s) with filter "%s" for %j',
                baseDN, truncateLogOutput(opts.filter), _.any(opts.attributes) ? opts.attributes : '[*]');
      opts.controls.push(new ldap.Control({ type: '1.2.840.113556.1.4.417', criticality: true }));
    }
    search.call(self, baseDN, opts, function onFind(err, results) {
      if (err) {
        if (callback) callback(err);
        return;
      }
      ...
      _.forEach(results, function(result) {
        self.emit('entry:deleted', item);
        result.push(item);
      });

      log.info('%d deleted objects found for query "%s". Results: %j',
               result.length, truncateLogOutput(opts.filter), result);
  ...
  var opts = _.defaults(opts || {}, {
    scope: 'base',
    attributes: [ 'attributeID', 'attributeSyntax', 'dnReferenceUpdate' , 'dNSHostName' , 'flatName',
  ...
  if (! opts.baseDN) {
    ...
    ActiveDirectory.prototype.getRootDSE(self.opts.url, [ 'defaultNamingContext' ], function(err, result) {
      ...
      searchDeletedObjects('ou=Deleted Objects,' + result.defaultNamingContext, opts);
    });
  }
  else searchDeletedObjects(opts.baseDN, opts);
};
 ...
 */
ActiveDirectory.prototype.findGroup = function findGroup(opts, groupName, findOpts, callback) {
  var self = this;
  ...
  if (typeof(groupName) === 'boolean') {
    findOpts = {includeGroupsMembership : groupName};
    groupName = opts;
  }
  ...
  log.trace('findGroup(%j,%s,%j)', opts, groupName, findOpts);
  ...
  search.call(self, opts, function onSearch(err, results) {
    ...
    if ((! results) || (results.length === 0)) {
      log.warn('Group "%s" not found for query "%s"', groupName, truncateLogOutput(opts.filter));
      if (callback) callback();
      return;
    }
    ...
    log.info('%d group(s) found for query "%s". Returning first group: %j',
             results.length, truncateLogOutput(opts.filter), group);
    // Also retrieving user group memberships?
    if (findOpts.includeGroupsMembership) {
      getGroupMembershipForDN.call(self, group.dn, function(err, groups) {
        if (err) {
 ...
 */
ActiveDirectory.prototype.findGroups = function findGroup(opts, findOpts, callback) {
  var self = this;
  ...
  log.trace('findGroups(%j,%j)', opts, findOpts);
  ...
  search.call(self, opts, function onSearch(err, results) {
    ...
    if ((! results) || (results.length === 0)) {
      log.warn('No groups found matching query "%s"', truncateLogOutput(opts.filter));
      if (callback) callback();
      return;
    }
    ...
    async.forEach(results, function(result, asyncCallback) {
      ...
      log.info('%d group(s) found for query "%s". Groups: %j', groups.length, truncateLogOutput(opts.filter), groups);
 ...
 */
ActiveDirectory.prototype.findUser = function findUser(opts, username, findOpts, callback) {
  var self = this;
  ...
  if (typeof(username) === 'boolean') {
    findOpts = {includeUsersMembership : username};
    username = opts;
  }
  ...
  log.trace('findUser(%j,%s,%j)', opts, username, findOpts);
  ...
  search.call(self, opts, function onSearch(err, results) {
    ...
    if ((! results) || (results.length === 0)) {
      log.warn('User "%s" not found for query "%s"', username, truncateLogOutput(opts.filter));
      if (callback) callback();
      return;
    }

    var user = new User(results[0]);
    log.info('%d user(s) found for query "%s". Returning first user: %j', results.length, truncateLogOutput(opts.filter), user);
 ...
 */
ActiveDirectory.prototype.findUsers = function findUsers(opts, findOpts, callback) {
  var self = this;
  ...
  log.trace('findUsers(%j,%j)', opts, findOpts);
  ...
  search.call(self, opts, function onSearch(err, results) {
    ...
    if ((! results) || (results.length === 0)) {
      log.warn('No users found matching query "%s"', truncateLogOutput(opts.filter));
      if (callback) callback();
      return;
    }
