...
function ActiveDirectory(options) { // jshint -W071
  ...
  Object.defineProperties(this, {
    ...
    baseDN: {value: options.baseDN},
    defaultAttributes: {
      value: Object.assign(
        {}, originalDefaultAttributes, options.attributes,
        (options.defaults) ? options.defaults.attributes : null
      )
    },
    defaultReferrals: {
      value: Object.assign(
        {}, originalDefaultReferrals, options.referrals,
        (options.defaults) ? options.defaults.referrals : null
      )
    }
  });
...
ActiveDirectory.prototype.getGroupMembershipForDN = function ggmfd(opts, dn, stack, callback) {
  return getGroupMembershipForDN.apply(this, arguments);
};
 ...
 *
 * @private
 * @param {LDAPQueryParameters} [opts] Optional LDAP query object.
 * @param {string} dn The distinguishedName to find groups for.
 * @param {function} callback The callback to invoke when completed.
 */
ActiveDirectory.prototype.getGroupMembershipForDN = function() {
  return getGroupMembershipForDN.apply(this, arguments);
};
...
ActiveDirectory.prototype.getDistinguishedNames = function gdns(opts, filter, callback) {
  return getDistinguishedNames.apply(this, arguments);
};
...
function getUserDistinguishedName(opts, username, callback) {
  ...
  this.getDistinguishedNames(opts, getUserQueryFilter(username), function(err, dns) {
    if (err) {
      callback(err);
      return;
    }
...
ActiveDirectory.prototype.getUserDistinguishedName = function gudn(opts, username, callback) {
  return getUserDistinguishedName.apply(this, arguments);
};
...
ActiveDirectory.prototype.getUsersForGroup = function getUsersForGroup(opts, groupName, callback) {
  ...
  self.findGroup(_.defaults({}, _.omit(opts || {}, 'attributes'), {
    ...
    async.each(chunks, function getUsersForGroup_ChunkItem(members, asyncChunkCallback) {
      ...
      search(localOpts, function onSearch(err, members) {
        if (err) {
...
ActiveDirectory.prototype.findGroup = function findGroup(opts, groupName, callback) {
  const groupFinder = require('./components/findGroup')(this, log);
  return groupFinder(opts, groupName, callback);
};
...
ActiveDirectory.prototype.findGroups = function findGroup(opts, callback) {
  ...
  search(localOpts, function onSearch(err, results) {
    if (err) {
      if (callback) callback(err);
      return;
    }
    ...
    async.forEach(results, function(result, asyncCallback) {
      if (isGroupResult(result)) {
        var group = new Group(pickAttributes(result, (opts || {}).attributes || defaultAttributes.group));
        groups.push(group);
...
ActiveDirectory.prototype.findUser = function findUser(opts, username, includeMembership, callback) {
  ...
  var errInvoked = false;
  search(localOpts, function onSearch(err, results) {
    if (err) {
      if (callback && !errInvoked) {
