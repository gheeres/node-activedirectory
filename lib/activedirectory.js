...
function getDistinguishedNames(opts, filter, callback) {
  ...
  opts = _.defaults(_.omit(opts || {}, 'attributes'), {
    filter: filter,
    scope: 'sub',
    attributes: _.union((opts || {}).attributes || [], [ 'distinguishedName' ])
  });
  search.call(self, opts, function(err, results) {
    if (err) {
      if (callback) callback(err);
      ...
      search.call(self, opts, function onSearch(err, members) {
        ...
        async.forEach(members, function(member, asyncCallback) {
          ...
          if (! member.groupType) {
            ...
            self.getUsersForGroup(_.omit(opts, 'filter', 'scope'), member.cn, function(err, nestedUsers) {
...
ActiveDirectory.prototype.find = function find(opts, callback) {
  ...
  opts = _.defaults(_.omit(opts || {}, 'attributes'), {
    ...
    attributes: _.union((opts || {}).attributes || [], defaultAttributes.group || [], defaultAttributes.user || [], 
                        getRequiredLdapAttributesForGroup(opts), getRequiredLdapAttributesForUser(opts), [ 'objectCategory' ])
  });
    ...
    async.forEach(results, function(item, asyncCallback) {
      if (isGroupResult(item)) {
        ...
        if (includeGroupMembershipFor(opts, 'group')) {
          getGroupMembershipForDN.call(self, _.omit(opts, 'scope'), group.dn, function(err, groups) {
        ...
        if (includeGroupMembershipFor(opts, 'user')) {
          getGroupMembershipForDN.call(self, _.omit(opts, 'scope'), user.dn, function(err, groups) {
            if (err) return(asyncCallback(err));
...
ActiveDirectory.prototype.findDeletedObjects = function find(opts, callback) {
  ...
  opts = _.defaults(opts || {}, {
    scope: 'one',
    attributes: _.union((opts || {}).attributes || [], [ 'attributeID', 'attributeSyntax', 'dnReferenceUpdate' , 'dNSHostName' , 'flatName',
  ...
  if (! opts.baseDN) {
    ...
    ActiveDirectory.prototype.getRootDSE(self.opts.url, [ 'defaultNamingContext' ], function(err, result) {
      ...
      searchDeletedObjects('CN=Deleted Objects,' + result.defaultNamingContext, opts);
    });
  }
...
ActiveDirectory.prototype.findGroup = function findGroup(opts, groupName, callback) {
  ...
  search.call(self, opts, function onSearch(err, results) {
    ...
    // Also retrieving user group memberships?
    if (includeGroupMembershipFor(opts, 'group')) {
      getGroupMembershipForDN.call(self, _.omit(opts, 'filter', 'scope'), group.dn, function(err, groups) {
        if (err) {
          if (callback) callback(err);
          return;
        }
...
ActiveDirectory.prototype.findUser = function findUser(opts, username, includeMembership, callback) {
  ...
  search.call(self, opts, function onSearch(err, results) {
    ...
    if (includeGroupMembershipFor(opts, 'user') || includeMembership) {
      getGroupMembershipForDN.call(self, _.omit(opts, 'filter', 'scope'), user.dn, function(err, groups) {
        if (err) {
          if (callback) callback(err);
          return;
        }
...
ActiveDirectory.prototype.findUsers = function findUsers(opts, includeMembership, callback) {
  ...
  search.call(self, opts, function onSearch(err, results) {
    ...
    async.forEach(results, function(result, asyncCallback) {
      ...
      // Also retrieving user group memberships?
      if (includeGroupMembershipFor(opts, 'user') || includeMembership) {
        getGroupMembershipForDN.call(self, _.omit(opts, 'attributes'), user.dn, function(err, groups) {
          if (err) return(asyncCallback(err));
