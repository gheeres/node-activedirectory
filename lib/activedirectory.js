...
ActiveDirectory.prototype.find = function find(opts, callback) {
  ...
  search.call(self, opts, function onFind(err, results) {
    ...
    if ((! results) || (results.length === 0)) {
      log.warn('No results found for query "%s"', truncateLogOutput(opts.filter), groups);
      if (callback) callback();
      self.emit('done');
      return;
    }
...
ActiveDirectory.prototype.getRootDSE = function getRootDSE(url, attributes, callback) {
  ...
  function onError(err) {
    // Ignore ECONNRESET errors
    if ((err || {}).errno !== 'ECONNRESET') {
      log.warn('An unhandled error occured when searching for the root DSE at "%s". Error: %s', url, err);
      return(callback(err));
    }
  }
