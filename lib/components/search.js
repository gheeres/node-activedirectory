...
const defaultPageSize = 1000; // should probably be a property of ad
let ad;
let log;
...
function createClient(url, opts) {
  ...
  log.trace('createClient(%s)', _url);

  const ldapOpts = getLdapClientOpts(Object.assign({ url: _url }, opts));
  log.debug('Creating ldapjs client for %s. Opts: %j', opts.url, ldapOpts);
 ...
 * @property {object} All of the options relevant to an {@link ActiveDirectory}
 * instance. It must include an `opts` property that represents that is
 * an {@link LDAPQueryParameters}.
...
function Searcher(opts) { // jshint -W071
  ...
  this.opts = opts;
  this.ldapOpts = getLdapClientOpts(this.opts);
  ...
  this.client = createClient(ad.url || ad.opts.url, this.ldapOpts);
