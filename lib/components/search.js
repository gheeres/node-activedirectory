...
Searcher.prototype.rangeSearch = function rangeSearch(query, rangeCB) {
  log.debug('Quering (%s) for range search with filter "%s" for: %j',
...
Searcher.prototype.onSearchEntry = function onSearchEntry(entry) {
  ...
  const parserCallback = (result) => {
    ...
    if (result && !this.results.has(result.dn)) {
      this.results.set(result.dn, result);
    }
    if (this.reachedEndOfRange) {
      this.rangeProcessing = false;
    }
    if (!this.rangeProcessing && this.searchComplete) {
      console.log('doing search end. results.length = %s', this.results.length);
      this.onSearchEnd();
    }
  };

  parseRangeAttributes(result, this, (err, item) => {
    console.log('parseRangeAttributes callback invoked');
    let _item = item;
    if (err) {
      _item = result;
    }
