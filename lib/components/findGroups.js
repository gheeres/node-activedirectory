...
GroupsFinder.prototype.asyncIterator = function asyncIterator(result, acb) {
  ...
  log.info('found group for query "%s". group: %j',
...
GroupsFinder.prototype.asyncCallback = function asyncCallback(err) {
  ...
  log.info('%d group(s) found for query "%s". Groups: %j',
...
GroupsFinder.prototype.onComplete = function onComplete(err, results) { // jshint -W071
  ...
  if (results.length === 0) {
    log.warn('Group not found for query "%s"', utils.truncateLogOutput(this.filter));
    if (this.callback) {
      this.callback(err, []);
    }
    return;
